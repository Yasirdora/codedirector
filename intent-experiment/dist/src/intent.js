...
exports.validateIntentObject = validateIntentObject;
exports.extractJson = extractJson;
exports.buildRepoContext = buildRepoContext;
exports.runIntentLayer = runIntentLayer;
...
function validateIntentObject(x) {
    const errs = [];
    ...
    for (const f of ["keep", "deny", "unknowns"]) {
        if (!Array.isArray(o[f]) || !o[f].every((s) => typeof s === "string"))
            errs.push(`${f}: must be a string array`);
    }
    if (typeof o.divergent !== "boolean")
        errs.push("divergent: must be a boolean");
    if (!Array.isArray(o.interpretations) || o.interpretations.length < 2) {
        ...
        for (const [i, it] of o.interpretations.entries()) {
            const r = it;
    ...
    if (o.divergent === true) {
        ...
        if (typeof q !== "object" || q === null) {
            ...
            if (!Array.isArray(q.options) || q.options.length < 2 || q.options.length > 4) {
                ...
                const ids = q.options.map((oo) => oo.id);
...
async function fileTree(root, maxEntries = 60) {
    const out = [];
...
function cdirCliPath() {
    return (process.env.CDIR_CLI ||
        node_path_1.default.resolve(__dirname, "..", "..", "..", "codedirector", "dist", "src", "cli.js"));
}
/**
 ...
 */
async function buildRepoContext(repoRoot, query) {
    ...
    try {
        ...
        const { stdout } = await execFileP(process.execPath, [cdirCliPath(), "map", query.slice(0, 200), "--root", repoRoot, "--top", "15", "--max-tokens", "900"], { timeout: 60_000 });
        ...
        try {
            ...
            for (const rel of tree.split("\n").filter((l) => l.endsWith(".ts"))) {
                const src = await node_fs_1.promises.readFile(node_path_1.default.join(repoRoot, rel), "utf8");
...
async function runIntentLayer(messyRequest, repoRoot, opts = {}) {
    const cfg = opts.cfg ?? (0, backend_1.backendConfigFromEnv)();
    let intentTokens = { ...backend_1.ZERO_USAGE };
    ...
    if (errs.length > 0)
        throw new Error(`intent object failed schema: ${errs.join("; ")}`);
    const intent = parsed;
