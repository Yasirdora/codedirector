...
interface Agg {
  ...
  unintendedMean: number;
  turnsMean: number;
  wallTimeMeanMs: number;
  agentTokensMean: number;
  intentTokensMean: number;
  totalTokensMean: number;
  questionRate: number;
  unnecessaryQuestions: number;
  missedQuestions: number;
  judgeAccuracyMean: number | null; // LLM-judged
  mechanicalProxyRate: number;
}
...
export interface Analysis {
  generatedAt: string;
  /** True when ANY record used the mock backend (computed from records). */
  mock: boolean;
  /** How many records used the mock backend. */
  mockRecords: number;
  repeats: number;
  perTask: Array<{ taskId: string; A: Agg; B: Agg }>;
  overall: { A: Agg; B: Agg };
  fewers: {
...
export function analyze(raw: RawFile): Analysis {
  ...
  const fmtAcc = (x: number | null) => (x === null ? "n/a (judge could not parse)" : x.toFixed(2));
  const fewers = {
    misunderstandings:
      `A mechanical-proxy ${A.mechanicalProxyRate.toFixed(2)} vs B ${B.mechanicalProxyRate.toFixed(2)}; ` +
      `LLM-judged intent accuracy A ${fmtAcc(A.judgeAccuracyMean)} vs B ${fmtAcc(B.judgeAccuracyMean)}.`,
    unnecessaryQuestions:
      `B asked questions in ${(B.questionRate * 100).toFixed(0)}% of runs; ` +
      `${B.unnecessaryQuestions} unnecessary, ${B.missedQuestions} missed (per oracle necessity labels). A asks none by construction.`,
    tokens:
      `mean agent tokens A ${A.agentTokensMean.toFixed(0)} vs B ${B.agentTokensMean.toFixed(0)}; ` +
  ...
  if (mockRecords > 0 && mockRecords < raw.records.length) {
    caveats.unshift(
      `MIXED PROVENANCE: ${mockRecords} of ${raw.records.length} records used the deterministic mock backend ` +
...
function mdTable(a: Analysis): string {
  const lines: string[] = [];
  ...
  for (const t of a.perTask) {
    for (const cond of ["A", "B"] as const) {
      const g = t[cond];
      lines.push(
        `| ${t.taskId} | ${cond} | ${(g.checksPassRate * 100).toFixed(0)}% | ${g.unintendedMean.toFixed(1)} | ` +
...
export async function analyzeFile(rawPath: string): Promise<Analysis> {
  const raw = JSON.parse(await fs.readFile(rawPath, "utf8")) as RawFile;
  const a = analyze(raw);
  const outDir = path.dirname(rawPath);
  await fs.writeFile(path.join(outDir, "results.json"), JSON.stringify(a, null, 2));
  await fs.writeFile(path.join(outDir, "results.md"), renderMarkdown(a));
...
if (isMain) {
  const p = process.argv[2] || path.join(__dirname, "..", "..", "results", "results.raw.json");
